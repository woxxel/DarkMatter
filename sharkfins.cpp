...
//         cout << "timeconstants: tau_M = " << mod.paras.tau_M << ", tau_G = " << mod.paras.tau_G << ", tau_A = " << mod.paras.tau_A << ", tau_N = " << mod.paras.tau_N << endl;
    ...
    if (sim.steps == 1) sim.initiate_y_axis(modP);
                                ...
                                if ((sim.mode_stats == 0) || (sim.mode_stats == 2) || (sim.mode_stats == 4))
                                {
                                    if (mod.paras.gamma[p] > 1) {
										if (!sim.trans_DM_found[p]) {
											// cout << " found DM transition for p=" << p << " and iter #" << sim.y_iter << ", @ rate=" << mod.paras.rate[p] << endl;
                                    ...
                                    if (mod.no_peak(p)) {
										if (!sim.trans_np_found[p]) {
											// cout << " found no peak transition for p=" << p << " and iter #" << sim.y_iter << ", @ rate=" << mod.paras.rate[p] << endl;
								...
								if (!sim.trans_inc_found) {
									...
									if ((sim.mode_stats==0) || (sim.mode_stats == 2) || (sim.mode_stats == 4))
									{
										for (int p = 0; p < mod.paras.Npop; p++)
										{
											...
											if (!sim.trans_np_found[p]) {
												// cout << "forcing np end @ rate = " << mod.paras.rate[p] << endl;
												// cout << "previous value: " << res.trans_np[p][sim.y_iter]<< endl;
...
//                                                     regions[sim.n_iter][sim.alpha_0_iter][sim.tau_G_iter][sim.rateWnt_iter][p] = val;
                            ...
                            // cout << "storing ..." << endl;
                            sim.store_results(simP,modP,resP);

                            // cout << "storing #2 ..." << endl;
                            if ((sim.mode_stats==3) || (sim.mode_stats==4))
                                sim.store_results_approx(simP, mod_approxP,resP);
...
// 	for (unsigned i=0;i<sim.steps;++i)
	if (sim.steps == 1)
	{
        ...
        if ((sim.mode_stats == 0) || (sim.mode_stats == 4))
            write_sharks(fileOut,simP,modP,resP);//,gammaP,chiP,regionsP);
        else
            write_stats(fileOut,modP,simP,resP);
    }
    // cout << "end" << endl;
	return 0;
}
