...
####    4: KL-phase-space (costly!)
    ...
    tau_M = 0.01
    inpara['tau_G'] = [[0.005],[0.01],[0.02],[0.04],[0.06]]
    ...
    # if (steps > 1):
        # if (mode_stats == 0):
    if (plt_para['multi']):
        # plot_3d(fig,ax[-1,-1],inpara,results,keys['x'],keys['const'],keys['y'],ax_labels,const_labels,from_disk=not plot_ax3D)
...
def plot_fins(ax,x_arr,y_arr,gamma,chi,regions,plt_para):
    ...
    plot_colorbar(pchi,pgamma,plt_para)
...
def plot_colorbar(pchi,pgamma,plt_para):
