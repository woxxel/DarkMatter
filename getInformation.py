...
def information(steps=100,
    rateWnt=[0,20],alpha_0=[0],tau_G=[0.005],eps=[0.5],eta=[0.9],n=[0],
    ...
    X,Y = np.meshgrid(X,Y)
    ax.plot_surface(X,Y,res['infoContent'][0,0,...])
    plt.setp(ax,zlim=[0,10],
        xlabel=r'$\displaystyle a$',
    ...
    for nu in [0.1,0.5,1,2,5]:
        ...
        print(nu,i)
        ax.plot(zeta_arr,res['infoContent'][0,0,i,:],label=r'$\displaystyle \nu=%.2g$'%nu)
    ax.legend()
    ...
    for zeta in [-3,-1.5,0,1.5,3]:
        ...
        print(zeta,i)
        ax.plot(nu_arr,res['infoContent'][0,0,:,i],label=r'$\displaystyle a=%.2g$'%zeta)
    ax.legend()
    plt.setp(ax,ylim=[0,10])
