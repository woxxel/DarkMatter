...
{	
	...
	double gamma[sim.n.size()][sim.alpha_0.size()][sim.tau_G.size()][sim.rateWnt.size()][2];
	double *gammaP = &gamma[0][0][0][0][0];
	double chi[sim.n.size()][sim.alpha_0.size()][sim.tau_G.size()][sim.rateWnt.size()][2];
...
// 			cout << "alpha: " << net.paras.alpha_0[0] << endl;
			for (unsigned tau_G_iter = 0; tau_G_iter < sim.tau_G.size(); tau_G_iter++)
			{	
				net.paras.tau_G = sim.tau_G[tau_G_iter];
// 				cout << "tau: " << net.paras.tau_G << endl;
...
// 					cout << "q: " << net.paras.q[0] << endl;
		...
		// 			cout << "solved: q_I=" << net.paras.q[0] << ", q_E=" << net.paras.q[1] << endl;
						...
						if (((1 < q_border1) || (1 < q_border2)) && (found_inconsistent[p] == 0))
						{
							...
							if (sim.rateWnt.size() > 1)
								for (unsigned f = rate_iter; f < sim.rateWnt.size(); f++)
									gamma[n_iter][alpha_iter][tau_G_iter][f][p] = val;
							else if (sim.tau_G.size() > 1)
								for (unsigned f = tau_G_iter; f < sim.tau_G.size(); f++)
									gamma[n_iter][alpha_iter][f][rate_iter][p] = val;
							else if (sim.alpha_0.size() > 1)
								for (unsigned f = alpha_iter; f < sim.alpha_0.size(); f++)
									gamma[n_iter][f][tau_G_iter][rate_iter][p] = val;
						}
					}
					if ((gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][0] == -1) and (gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][1] == -1))
						break;
					...
					{  
						...
						gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][p] = net.paras.sigma_V[p]/net.paras.alpha[p];
						
						if(gsl_pow_2(gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][p])*gsl_pow_2(delta)-4*(gsl_pow_2(gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][p]) - 1) < 0)
						{
							found_no_peak[p] = 1;
		// 					cout << "no peak" << endl;
		// 					for (unsigned k = j; k < sim.rateWnt.size(); k++)
							gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][p] = -2;			// entries -1 mean: inconsistent
						}
						else
						{
							double nu_peak_log_I = nu_peak_log_full(gamma[n_iter][alpha_iter][tau_G_iter][rate_iter][p],delta,net.paras.rate_max[p]);
							chi[n_iter][alpha_iter][tau_G_iter][rate_iter][p] = -log10(exp(1)) * nu_peak_log_I + log10(net.paras.rateWnt[0]);
						}
					}
				}
			}
